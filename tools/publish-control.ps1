...
$TargetPath = Join-Path $RepoRoot "apps\control"

if (-not (Test-Path $SourcePath)) {
    throw "No existe la carpeta de Control: $SourcePath"
}
...
foreach ($RequiredFile in $RequiredFiles) {
    ...
    if (-not (Test-Path $RequiredPath)) {
        throw "La carpeta indicada no parece ser Control completo. Falta: $RequiredFile"
    }
}
...
try {
    ...
    if ($Package.name -ne "deliver-assets-control") {
        throw "El paquete copiado no es Control. Nombre encontrado: $($Package.name)"
    }
    ...
    if ($AndroidPackage -ne "com.deliverassets.control") {
        throw "El identificador Android no corresponde a Control. Encontrado: $AndroidPackage"
    }

    $PackageVersion = [string]$Package.version

    git add --all -- "apps/control"
    if ($LASTEXITCODE -ne 0) {
        throw "No se pudieron preparar los archivos de Control."
    }

    $Pending = git status --porcelain -- "apps/control"
    if ([string]::IsNullOrWhiteSpace(($Pending -join ""))) {
        Write-Host "Control ya está actualizado; no hay cambios para publicar."
        exit 0
    }

    git commit -m "feat(control): publish Control v$PackageVersion"
    if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "Control v$PackageVersion publicada correctamente."
