...
$TargetPath = Join-Path $RepoRoot "apps\rider"

if (-not (Test-Path $SourcePath)) {
    throw "No existe la carpeta de Rider: $SourcePath"
}
...
foreach ($RequiredFile in $RequiredFiles) {
    ...
    if (-not (Test-Path $RequiredPath)) {
        throw "La carpeta indicada no parece ser Rider completo. Falta: $RequiredFile"
    }
}
...
try {
    ...
    if ($Package.name -ne "deliver-assets-rider") {
        throw "El paquete copiado no es Rider. Nombre encontrado: $($Package.name)"
    }
    ...
    if ($AndroidPackage -ne "com.deliverassets.rider") {
        throw "El identificador Android no corresponde a Rider. Encontrado: $AndroidPackage"
    }

    $PackageVersion = [string]$Package.version

    git add --all -- "apps/rider"
    if ($LASTEXITCODE -ne 0) {
        throw "No se pudieron preparar los archivos de Rider."
    }

    $Pending = git status --porcelain -- "apps/rider"
    if ([string]::IsNullOrWhiteSpace(($Pending -join ""))) {
        Write-Host "Rider ya está actualizado; no hay cambios para publicar."
        exit 0
    }

    git commit -m "feat(rider): publish Rider v$PackageVersion"
    if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "Rider v$PackageVersion publicada correctamente."
