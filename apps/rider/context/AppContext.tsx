...
export function AppProvider({ children }: PropsWithChildren) {
  ...
  const getOperationCoordination = (id: string): OperationCoordination | null => {
    const operation = operations.find((item) => item.id === id && item.riderId === riderId)
  ...
  const sendOperationText = (id: string, value: string): ActionResult => {
    ...
    const operation = operations.find((item) => item.id === id && item.riderId === riderId)
    if (!operation) return { ok: false, message: 'El pedido no está asignado a este repartidor.' }
    ...
    queueAction('chat_message', id, 'Mensaje enviado por repartidor', { messageId: message.id, type: 'text' })
  ...
  const sendOperationImage = (id: string, imageUrl: string): ActionResult => {
    ...
    const message = createCoordinationMessage(
      ...
      riderProfile.name,
      'image',
      { text: 'Fotografía de recojo o entrega', imageUrl },
    )
    setOperations((current) => current.map((item) => item.id === id ? {
      ...item,
      updatedAt: message.createdAt,
    ...
    queueAction('chat_message', id, 'Fotografía enviada por repartidor', { messageId: message.id, type: 'image' })
  ...
  const escalateOperationChat = (id: string): ActionResult => {
    ...
    const updated = escalateCoordination(operation, riderProfile.name)
    setOperations((current) => current.map((item) => item.id === id ? {
      ...item,
      updatedAt: updated.escalatedAt ?? item.updatedAt,
      coordination: updated,
    } : item))
    queueAction('chat_escalation', id, 'Repartidor solicitó ayuda de Control', { actor: 'rider' })
    return { ok: true, message: 'Control fue solicitado y el caso quedó marcado para revisión.' }
  }
  ...
  const acceptRiderOffer = (id: string): ActionResult => {
    ...
    setOperations((current) => current.map((item) => {
      if (item.id !== offer.orderId) return item
      const assigned = { ...item, riderName: riderProfile.name, riderId }
      const progressed = appendOperationEvent(assigned, 'rider_assigned', 'rider')
      const joined = appendCoordinationParticipant(progressed, {
        role: 'rider',
        id: riderId,
        name: riderProfile.name,
        joinedAt: progressed.updatedAt,
      })
      const joinedMessage = createCoordinationMessage(
        ...
        'DELIVER ASSETS',
        'system',
        { text: `${riderProfile.name} fue asignado y se unió a la coordinación del pedido.`, createdAt: progressed.updatedAt },
      )
      return {
        ...progressed,
        coordination: {
          ...joined,
          messages: mergeCoordinationMessages(joined.messages, [joinedMessage]),
        },
      }
    }))
