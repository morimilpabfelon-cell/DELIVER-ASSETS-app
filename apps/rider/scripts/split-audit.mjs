...
check('paquete Android exclusivo',app.android.package==='com.deliverassets.rider');
check('bundle iOS exclusivo',app.ios.bundleIdentifier==='com.deliverassets.rider');
check('rol fijo en auth',fs.readFileSync(path.join(root,'app/auth.tsx'),'utf8').includes("const APP_ROLE: AppRole = 'repartidor'"));
check('sin selector de cuatro roles',!fs.existsSync(path.join(root,'app/role-select.tsx')));
check('namespace de persistencia exclusivo',fs.readFileSync(path.join(root,'services/persistence.ts'),'utf8').includes("@deliver-assets/rider-state-v2"));
check('conserva app/rider.tsx',fs.existsSync(path.join(root,'app/rider.tsx')));
