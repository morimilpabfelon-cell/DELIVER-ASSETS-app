...
if (-not (Test-Path (Join-Path $Root "node_modules\expo\package.json"))) {
  Write-Host "Instalando dependencias de Business..." -ForegroundColor Yellow
  npm ci
}
...
if ($OldHub) {
  Write-Host "Reemplazando una versión antigua del Sync Hub..." -ForegroundColor Yellow
  try {
...
Write-Host "Business activo en 8082; Sync Hub compartido activo en 9090." -ForegroundColor Green
