...
export type AppState = {
  ...
  businessProfiles: Record<number, BusinessLocalProfile>
  currentBusinessProfile: BusinessLocalProfile
  updateBusinessProfile: (storeId: number, patch: Partial<Omit<BusinessLocalProfile, 'storeId'>>) => void
...
export function AppProvider({ children }: PropsWithChildren) {
  ...
  const merchantStatesRef = useRef<Record<number, MerchantStoreState>>(createMerchantStates())
  ...
  const [businessProfiles, setBusinessProfiles] = useState<Record<number, BusinessLocalProfile>>(createBusinessProfiles)
  ...
  const currentBusinessProfile = businessProfiles[currentMerchantStoreId] ?? createBusinessProfiles()[currentMerchantStoreId]
  const merchantProfile = useMemo(() => {
    const canonical = storeProfile(currentMerchantStoreId)
    const local = businessProfiles[currentMerchantStoreId]
    return local ? { ...canonical, email: local.email, phone: local.phone, subtitle: local.description } : canonical
  }, [businessProfiles, currentMerchantStoreId])
    ...
    const hydrate = async () => {
      try {
        ...
        if (saved) {
          ...
          const restoredMerchantStates = normalizeMerchantStates(saved.merchantStates)
          setMerchantStates(restoredMerchantStates)
          merchantStatesRef.current = restoredMerchantStates
          const restoredBusinessProfiles = saved.businessProfiles ?? createBusinessProfiles()
          setBusinessProfiles(Object.fromEntries(Object.entries(restoredBusinessProfiles).map(([key, item]) => [key, {
            ...item,
            logoUri: item.logoUri && businessMediaExists(item.logoUri) ? item.logoUri : null,
            coverUri: item.coverUri && businessMediaExists(item.coverUri) ? item.coverUri : null,
            productMedia: Object.fromEntries(Object.entries(item.productMedia ?? {}).map(([productId, media]) => [productId, {
              ...media,
              localUri: media.localUri && businessMediaExists(media.localUri) ? media.localUri : null,
              status: media.publicUrl ? 'published' : media.localUri && businessMediaExists(media.localUri) ? media.status : 'empty',
            }])),
          }])))
  ...
  const applyHubState = useCallback((state: SharedHubState, authoritative: boolean, preserveMerchantState = false) => {
    ...
    if (state.merchantStates && !preserveMerchantState && (authoritative || !merchantDomainDirtyRef.current)) {
      setMerchantStates((current) => {
        const normalized = normalizeMerchantStates(state.merchantStates as Record<number, Partial<MerchantStoreState>>)
        const next = sameJson(current, normalized) ? current : normalized
  ...
  const performHubSync = useCallback(async (reason: 'auto' | 'manual' | 'queue', idempotencyKey?: string): Promise<boolean> => {
    ...
    const storedSnapshot = snapshotRef.current
    if (!storedSnapshot) return false
    const currentSnapshot: PersistedAppSnapshot = {
      ...storedSnapshot,
      merchantStates: merchantStatesRef.current,
      merchantStock: merchantStatesRef.current[currentMerchantStoreId]?.stock ?? storedSnapshot.merchantStock,
      updatedAt: new Date().toISOString(),
    }
    ...
    try {
      ...
      const preserveMerchantState = merchantWasDirty && backendConfig.appId === 'business' && !result.accepted.merchants
      applyHubState(result.state, true, preserveMerchantState)
      if (preserveMerchantState) {
        merchantDomainDirtyRef.current = true
        setPendingActions((current) => current.filter((action) => action.kind === 'merchant_settings'))
        setSyncStatus('pending')
        setSyncMessage('El Hub tenía una revisión comercial más nueva. Business conservó tu cambio y lo reintentará sin revertir el inventario.')
        return false
      }
      setPendingActions([])
  ...
  }, [applyHubState, currentMerchantStoreId, logTechnicalEvent, networkMode])
  ...
  const runHealthCheck = useCallback(async (): Promise<HealthCheck[]> => {
    ...
    const checks: HealthCheck[] = [
      ...
      { id: 'business-profiles', label: 'Perfiles comerciales locales', ok: Object.keys(businessProfiles).length === stores.length, detail: `${Object.keys(businessProfiles).length}/${stores.length} perfiles disponibles.` },
    ...
    return checks
  }, [activeOperationId, addresses, hubConnected, hubRevision, logTechnicalEvent, businessProfiles, merchantStates, operations, payments, pendingActions, reducedMotion, snapshot])
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(timer)
  }, [guestSession, hubConnected, hubRevision, hydrated, lastLifecycleState, networkMode, pendingActions, performHubSync, sharedSignature])
  ...
  const updateBusinessProfile = useCallback((storeId: number, patch: Partial<Omit<BusinessLocalProfile, 'storeId'>>) => {
    if (!stores.some((store) => store.id === storeId)) return
    setBusinessProfiles((current) => ({
      ...current,
      [storeId]: {
        ...(current[storeId] ?? createBusinessProfiles()[storeId]),
        ...patch,
        storeId,
        updatedAt: new Date().toISOString(),
      },
    }))
    logTechnicalEvent('success', 'business-profile', `Perfil local del comercio ${storeId} actualizado.`)
  }, [logTechnicalEvent])
  const updateMerchantState = (patch: Partial<Omit<MerchantStoreState, 'storeId'>>) => {
    ...
    const next = { ...current, [currentMerchantStoreId]: { ...base, ...patch } }
    merchantStatesRef.current = next
  ...
  const resetDemoData = useCallback(async () => {
    ...
    clearAllBusinessMedia(); setCurrentMerchantStoreId(1); const resetMerchantStates = createMerchantStates(); setMerchantStates(resetMerchantStates); merchantStatesRef.current = resetMerchantStates; setBusinessProfiles(createBusinessProfiles()); setRiderOnline(false); setRiderVoiceNavigation(true); setRiderRouteStage(0); setActiveRiderOffer(null)
  ...
  return <AppContext.Provider value={{
    ...
    role, authenticated, guestSession, setRole, authenticate, logout, profile, updateProfile, merchantProfile, businessProfiles, currentBusinessProfile, updateBusinessProfile, riderProfile, adminProfile,
