...
export default function DataCenter() {
  ...
  const {
    ...
    saveNow, syncNow, resetDemoData, merchantOrders, operations, merchantStates, businessProfiles, pendingActions, currentMerchantStoreId,
  } = useApp()
  ...
  const runSync = async () => {
    ...
    const message = ok
      ? backendMode === 'local' ? 'Los datos comerciales locales fueron verificados en este dispositivo.' : 'El servidor confirmó la sincronización del estado actual.'
  ...
  const reset = () => Alert.alert('Restaurar datos demo', 'Se borrarán perfiles comerciales, imágenes administradas, inventario, pedidos demo y configuraciones locales de Business.', [
    { text: 'Cancelar', style: 'cancel' },
    { text: 'Restaurar', style: 'destructive', onPress: () => void resetDemoData().then(() => router.replace('/auth')) },
  ])
  ...
  return <SafeAreaView style={styles.safe} edges={['top','bottom']}><Header title="DATOS Y SINCRONIZACIÓN" kicker="ARQUITECTURA MULTI-APP · V2.3" onBack={() => router.back()}/><ScrollView contentContainerStyle={styles.content}>
    ...
    <View style={styles.records}>
      {[
        ['storefront-outline', 'Comercios configurados', Object.keys(merchantStates).length, C.yellow],
        ['person-circle-outline', 'Perfiles comerciales', Object.keys(businessProfiles).length, C.blue],
        ['receipt-outline', 'Órdenes del comercio activo', merchantOrders.length, C.red],
        ['git-branch-outline', 'Operaciones integradas', operations.length, C.mint],
        ['cloud-upload-outline', 'Cambios pendientes', pendingActions.length, C.yellow],
        ['key-outline', 'Comercio activo ID', currentMerchantStoreId, C.blue],
