...
export async function pullHubState(options: Pick<SyncOptions, 'timeoutMs' | 'simulatedDelayMs'> = {}): Promise<SyncResult> {
  ...
  try {
    const response = await requestJson(`${apiUrl.replace(/\/$/, '')}/v1/state`, { method: 'GET', headers: { 'X-DA-Client-Version': '2.3.0-control' } }, timeoutMs)
    ...
    try {
      const response = await requestJson(`${apiUrl.replace(/\/$/, '')}/v1/sync`, {
        ...
        headers: {
          ...
          'X-DA-Client-Version': '2.3.0-control',
        },
