...
import type { ActiveOrder, CartLine, DeliveryKind, MerchantStoreState, Shipment, UserProfile, UserSettings } from '@/context/AppContext'

export const APP_STORAGE_KEY = '@deliver-assets/control-state-v2'
...
function normalizeSnapshot(value: unknown): PersistedAppSnapshot | null {
  ...
  if (![1, 2, 3, 4, 5, 6, APP_SCHEMA_VERSION].includes(candidate.schemaVersion ?? -1)) return null
  ...
  if (candidate.merchantStates && typeof candidate.merchantStates === 'object') {
    for (const store of stores) {
      ...
      if (saved) states[store.id] = { ...states[store.id], ...saved, stock: { ...states[store.id].stock, ...saved.stock } }
    }
  } else {
  ...
  return {
    ...
    guestSession: false,
    profile: candidate.profile ?? { name: 'Eidon Morimil', email: 'eidon@deliverassets.demo', phone: '+51 999 432 101' },
    ...
    cart: Array.isArray(candidate.cart) ? candidate.cart : [],
