...
import { categories, CategoryKey, categoryByKey, stores } from '@/data/catalog'
...
export default function Search() {
  ...
  const results = useMemo(() => {
    ...
    return stores.filter((store) => {
      ...
      const searchable = `${store.name} ${store.descriptor} ${store.products.map((product) => `${product.name} ${product.group} ${(product.tags ?? []).join(' ')}`).join(' ')}`.toLowerCase()
  ...
  return <SafeAreaView style={styles.safe} edges={['top']}>
    <View style={[styles.header, { backgroundColor: meta ? tone(meta.tone) : C.mint }]}>
      <Text style={styles.kicker}>BÚSQUEDA POR UNIVERSO</Text><Text style={styles.title}>{meta ? `SOLO\n${meta.label.toUpperCase()}.` : 'ENCUENTRA\nLO QUE QUIERAS.'}</Text>
      <View style={styles.input}><Ionicons name="search" size={20}/><TextInput autoFocus value={q} onChangeText={setQ} placeholder={meta?.searchPlaceholder ?? 'Tienda, categoría o producto'} placeholderTextColor={C.gray} style={styles.inputText}/>{q.length > 0 && <Pressable onPress={() => setQ('')}><Ionicons name="close-circle" size={20}/></Pressable>}</View>
    </View>
    ...
    <ScrollView contentContainerStyle={styles.content}>
      ...
      {results.map((store) => <Pressable key={store.id} onPress={() => router.push({ pathname: '/store/[id]', params: { id: String(store.id) } })} style={styles.row}><View style={[styles.symbol, { backgroundColor: tone(store.tone) }]}><Text style={styles.symbolText}>{store.symbol}</Text></View><View style={{ flex: 1 }}><Text style={styles.categoryLabel}>{categoryByKey(store.category).label.toUpperCase()}</Text><Text style={styles.name}>{store.name.toUpperCase()}</Text><Text style={styles.desc}>{store.descriptor}</Text><Text style={styles.meta}>{store.eta} · ★ {store.rating}</Text></View><Ionicons name="arrow-forward" size={22}/></Pressable>)}
