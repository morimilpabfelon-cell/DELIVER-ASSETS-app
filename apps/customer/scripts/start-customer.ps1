...
if (-not (Test-Path (Join-Path $Root "node_modules\expo\package.json"))) {
  Write-Host "Instalando dependencias de Customer..." -ForegroundColor Yellow
  npm ci
}
...
Write-Host "Customer activo en 8081; Sync Hub compartido activo en 9090." -ForegroundColor Green
