...
export function OperationReceiptCard({
  ...
  return <View style={[styles.card, compact && styles.compact]}>
    <View style={styles.head}>
      <View>
        <Text style={styles.kicker}>BOLETA DE VENTA</Text>
        <Text style={styles.number}>{receipt.number}</Text>
      </View>
      <View style={[styles.payment, receipt.paymentState === 'captured' ? styles.paid : receipt.paymentState === 'cash_due' ? styles.cash : styles.pending]}>
        <Text style={styles.paymentText}>{paymentStateLabel(receipt.paymentState)}</Text>
      </View>
    </View>

    {!compact && <>
      <View style={styles.identity}>
        <View style={{ flex: 1 }}><Text style={styles.label}>NEGOCIO</Text><Text style={styles.value}>{receipt.merchantName}</Text></View>
        <View style={{ flex: 1 }}><Text style={styles.label}>CLIENTE</Text><Text style={styles.value}>{receipt.customerName}</Text></View>
      </View>
    ...
    <View style={styles.total}>
      <View><Text style={styles.label}>TOTAL PAGADO / POR COBRAR</Text><Text style={styles.method}>{receipt.paymentLabel}</Text></View>
...
const styles = StyleSheet.create({
  ...
  head: { padding: 12, flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, borderBottomWidth: 2, borderColor: C.black, backgroundColor: C.paper },
  kicker: { fontSize: 7, fontWeight: '900', letterSpacing: 1 },
  number: { marginTop: 5, fontSize: 13, fontWeight: '900' },
  payment: { maxWidth: 132, paddingHorizontal: 8, paddingVertical: 6, borderWidth: 2, borderColor: C.black },
  ...
  line: { minHeight: 54, paddingVertical: 9, flexDirection: 'row', gap: 10, alignItems: 'flex-start', borderBottomWidth: 1, borderColor: C.line },
  ...
  total: { minHeight: 72, padding: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 10, borderTopWidth: 2, borderColor: C.black, backgroundColor: C.black },
