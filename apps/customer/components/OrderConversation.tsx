...
export function OrderConversation({
  ...
  if (!operation || !coordination) {
    return <View style={styles.missing}>
      <Ionicons name="chatbubble-ellipses-outline" size={42}/>
      <Text style={styles.missingTitle}>CONVERSACIÓN NO ENCONTRADA.</Text>
      <Text style={styles.missingCopy}>Sincroniza el pedido e inténtalo nuevamente.</Text>
    </View>
  }
  ...
  const sendText = () => {
    ...
    if (!result.ok) {
      showToast({ title: 'Mensaje no enviado', message: result.message, tone: 'warning' })
      return
    }
    setDraft('')
  }
  ...
  const uploadAsset = async (asset: ImagePicker.ImagePickerAsset) => {
    ...
    const result = await publishChatImage(operation.id, uploadId, asset.uri, clientVersion, asset.mimeType ?? 'image/jpeg')
    setUploading(false)
    if (!result.ok) {
      showToast({ title: 'Fotografía pendiente', message: result.message, tone: 'error', duration: 4200 })
      return
    }
  ...
  const fromGallery = async () => {
    ...
    if (!permission.granted) {
      showDialog({ title: 'Acceso a fotos', message: 'Autoriza el acceso a tus fotos desde Android para adjuntar una imagen.', tone: 'warning' })
      return
    }
  ...
  const fromCamera = async () => {
    ...
    if (!permission.granted) {
      showDialog({ title: 'Acceso a cámara', message: 'Autoriza la cámara desde Android para tomar una evidencia.', tone: 'warning' })
      return
    }
  ...
  >
    <View style={styles.status}>
      <View style={{ flex: 1 }}>
        ...
        <Text style={styles.statusCopy}>{operation.merchantName} · {paymentStateLabel(operation.paymentState)}</Text>
      </View>
    ...
    <View style={styles.actions}>
      <Pressable onPress={onOpenReceipt} style={styles.actionButton}>
        <Ionicons name="receipt-outline" size={17}/><Text style={styles.actionText}>BOLETA</Text>
      </Pressable>
      <Pressable onPress={() => void syncNow()} style={styles.actionButton}>
        <Ionicons name="sync-outline" size={17}/><Text style={styles.actionText}>SYNC</Text>
      </Pressable>
      {!closed && <Pressable onPress={escalate} style={[styles.actionButton, coordination.escalated && styles.escalated]}>
        <Ionicons name="shield-outline" size={17}/><Text style={styles.actionText}>{coordination.escalated ? 'CONTROL AVISADO' : 'PEDIR AYUDA'}</Text>
      </Pressable>}
    </View>

    {coordination.escalated && <View style={styles.escalation}>
      <Ionicons name="shield-checkmark" size={19}/>
      <Text style={styles.escalationText}>CONTROL FUE SOLICITADO. LA CONVERSACIÓN QUEDÓ MARCADA PARA REVISIÓN.</Text>
    </View>}
    ...
    >
      ...
      {closed && <View style={styles.closed}>
        ...
        <Text style={styles.closedCopy}>El pedido finalizó. El chat permanece visible como historial y no admite nuevos mensajes.</Text>
      </View>}
    </ScrollView>
    ...
    {!closed && <View style={styles.composer}>
      ...
      <TextInput
        ...
        placeholder="Escribe sobre este pedido…"
    ...
    <Modal visible={Boolean(preview)} transparent animationType="fade" onRequestClose={() => setPreview(null)}>
      <View style={styles.previewBackdrop}>
        <Pressable accessibilityLabel="Cerrar fotografía" style={styles.previewClose} onPress={() => setPreview(null)}>
...
function Message({
  ...
  if (message.type === 'receipt') {
    return <Pressable onPress={onOpenReceipt}>
      <OperationReceiptCard receipt={receipt} compact/>
    </Pressable>
  }
  ...
  if (message.senderRole === 'system') {
    return <View style={styles.systemMessage}>
      ...
      <Text style={styles.systemTime}>{time(message.createdAt)}</Text>
    </View>
  }
...
const styles = StyleSheet.create({
  ...
  missingCopy: { marginTop: 7, color: C.gray, fontSize: 9, textAlign: 'center' },
  status: { minHeight: 82, padding: 12, flexDirection: 'row', alignItems: 'center', gap: 10, borderBottomWidth: 2, borderColor: C.black, backgroundColor: C.black },
  ...
  actions: { minHeight: 48, paddingHorizontal: 9, flexDirection: 'row', alignItems: 'center', gap: 6, borderBottomWidth: 2, borderColor: C.black, backgroundColor: C.white },
  actionButton: { minHeight: 32, paddingHorizontal: 8, flexDirection: 'row', alignItems: 'center', gap: 5, borderWidth: 1, borderColor: C.black, backgroundColor: C.paper },
  actionText: { fontSize: 6, fontWeight: '900' },
  escalated: { backgroundColor: C.yellow },
  escalation: { minHeight: 55, padding: 10, flexDirection: 'row', alignItems: 'center', gap: 8, borderBottomWidth: 2, borderColor: C.black, backgroundColor: C.yellow },
  escalationText: { flex: 1, fontSize: 7, fontWeight: '900', lineHeight: 11 },
  messages: { padding: 12, paddingBottom: 25 },
  systemMessage: { alignSelf: 'center', maxWidth: '92%', marginVertical: 6, paddingHorizontal: 10, paddingVertical: 7, borderWidth: 1, borderColor: C.black, backgroundColor: C.paper },
  ...
  bubble: { maxWidth: '86%', padding: 9, borderWidth: 2, borderColor: C.black, backgroundColor: C.white },
  ...
  messageImage: { width: 230, height: 172, marginTop: 7, borderWidth: 1, borderColor: C.black, backgroundColor: C.paper },
  closed: { minHeight: 120, marginTop: 15, padding: 16, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: C.black, backgroundColor: C.white },
  ...
  composer: { minHeight: 72, padding: 8, flexDirection: 'row', alignItems: 'flex-end', gap: 7, borderTopWidth: 2, borderColor: C.black, backgroundColor: C.white },
  mediaButton: { width: 46, height: 46, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: C.black, backgroundColor: C.yellow },
  input: { flex: 1, minHeight: 46, maxHeight: 110, paddingHorizontal: 10, paddingVertical: 10, borderWidth: 2, borderColor: C.black, backgroundColor: C.paper, fontSize: 10, textAlignVertical: 'top' },
  send: { width: 46, height: 46, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: C.black, backgroundColor: C.black },
  sendDisabled: { opacity: .32 },
  previewBackdrop: { flex: 1, padding: 20, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(0,0,0,.92)' },
  previewClose: { position: 'absolute', right: 18, top: 45, zIndex: 2, width: 46, height: 46, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: C.black, backgroundColor: C.white },
